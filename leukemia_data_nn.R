...
library("caret")
TrainingParameters <- trainControl(classProbs=T)
set.seed(1120)
library(doMC)
registerDoMC(10)
nnModel <- train(mat_train, y_train,
                 method = "nnet",
...
TrainingParameters <- trainControl(method = "repeatedcv", number = 10, repeats=1,ret)
...
pdf("Accuracy_nn_leukemia.pdf",height = 6, width = 10)
plot(nnModel)
dev.off()

saveRDS(nnModel,"nnModel.rds")
